...
A natural next step is to apply classical scheduling theory to this new domain. In traditional OS scheduling, Shortest Job First (SJF) provably minimizes average waiting time when job sizes are known~\cite{silberschatz2018os}. LLM requests carry rich metadata-prompt length, token budget and conversation depth that might serve as job size proxies. This motivates our central research question i.e. -

\begin{quote}
\textit{Do classical OS scheduling assumptions like observable job sizes and sequential execution, hold in LLM agent systems? If not, what are the implications for scheduler design?}
\end{quote}

We hypothesize that request level features can approximate job size thus enabling classical scheduling improvements over FIFO. We test this hypothesis through three progressively refined approaches and ultimately \textit{reject} it, providing a structural explanation rooted in GPU batch execution dynamics. While prior work establishes the need for scheduling in LLM agent systems~\cite{mei2025aios}, our results reveal a fundamental limitation: \textit{under GPU batching, request-level scheduling provides negligible benefit over FIFO}.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{model_comparison_latency.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{correlation_bars.png}}
...
Since continuous prediction kept failing , so we attempted discrete classification into priority based classes.

\subsection{Setup}
We define three classes using percentile thresholds: \texttt{fast} ($< p_{33} = 3,846$ms), \texttt{medium} ($p_{33}$--$p_{66}$), \texttt{large} ($\geq p_{66} = 6,993$ms). The feature vector includes 5 numeric features i.e. the 5 agent type one hot indicators, and 2 model indicators (which include 12 features in totality). We evaluate six classifiers with stratified 5-fold CV on 80\% of the data.
...
After hyperparameter tuning (432  candidate grid search), we observe that Gradient Boosting achieves held out F1 = 0.47 (with an accuracy of 0.47). Feature importances: \texttt{input\_char\_length} (0.36), \texttt{max\_tokens} (0.24), \texttt{agent\_code\_gen} (0.10). The ``medium'' class is poorly discriminated (F1 = 0.41), while ``fast'' (F1 = 0.47) and ``large'' (F1 = 0.52) show partial separability.

\subsection{Analysis: Why Classification is Weak}

The F1 of 0.47 (vs. 0.33 random baseline) represents a 42\% improvement over chance but at the same time remains insufficient for reliable scheduling. The root cause is identical to Phase 1: latency classes are defined by \textit{actual execution time}, which depends on batch dynamics and output length, not input features. The classifier learns a noisy approximation of the max\_tokens $\rightarrow$ latency relationship, which is the only signal available.

\subsection{Request Type Classification}

The latency classifier's poor performance raises a question: \textit{is the feature space itself unstructured, or is latency simply the wrong target?} To answer this, we define a 4-class \textbf{request type taxonomy} based on structural reasoning patterns:

\begin{itemize}
    \item \textbf{Simple QA}: \texttt{max\_tokens} $\leq$ 128, single-turn, no tools
    \item \textbf{Conversational}: multi-turn (\texttt{message\_count} $>$ 2), no tools
    \item \textbf{Tool-Augmented}: \texttt{has\_tools} = true
    \item \textbf{Long Generation}: \texttt{max\_tokens} $>$ 512, single-turn, no tools
\end{itemize}

Critically, we \textit{exclude} \texttt{max\_tokens} and \texttt{has\_tools} from the feature vector, forcing the classifier to learn from proxy signals (\texttt{input\_char\_length}, \texttt{temperature}, \texttt{agent\_type}, \texttt{model\_name}, \texttt{message\_count}). Data is split 60/20/20 into train, validation, and held-out test sets.

\begin{table}[htbp]
\caption{Request Type Classifier (Test Set, N=454)}
\begin{center}
\begin{tabular}{lcccc}
\toprule
\textbf{Class} & \textbf{Precision} & \textbf{Recall} & \textbf{F1} & \textbf{Support} \\
\midrule
Simple QA & 0.66 & 0.75 & 0.70 & 169 \\
Conversational & 1.00 & 0.99 & 1.00 & 109 \\
Tool-Augmented & 1.00 & 1.00 & 1.00 & 91 \\
Long Generation & 0.33 & 0.25 & 0.28 & 85 \\
\midrule
\textbf{Macro Avg} & \textbf{0.75} & \textbf{0.75} & \textbf{0.75} & 454 \\
\bottomrule
\end{tabular}
\label{tab:requesttype}
\end{center}
\end{table}

The tuned KNN classifier ($k$=3) achieves \textbf{F1 = 0.75} on the held-out test set (Table~\ref{tab:requesttype}). Tool-Augmented and Conversational requests are classified perfectly (F1 = 1.00), because \texttt{agent\_type} and \texttt{message\_count} serve as strong proxy signals. Simple QA achieves F1 = 0.70. Long Generation is the hardest class (F1 = 0.28), since distinguishing it from Simple QA requires knowing \texttt{max\_tokens}, which is excluded from features.

Fig.~\ref{fig:confusion} shows the confusion matrix. The off-diagonal mass is concentrated in a single cell: 64 Long Generation requests misclassified as Simple QA. Both classes are single-turn with no tools; without \texttt{max\_tokens}, they are structurally identical to the classifier. Conversational and Tool-Augmented classes show near-zero confusion, confirming that \texttt{message\_count} and \texttt{agent\_type} are reliable discriminators.

\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{reasoning_confusion_matrix.png}}
\caption{Confusion matrix for the 4-class request type classifier (held-out test set). Conversational and Tool-Augmented are classified perfectly. The primary confusion is between Simple QA and Long Generation, which are structurally indistinguishable without \texttt{max\_tokens}.}
\label{fig:confusion}
\end{figure}

\textbf{Insight.} The contrast between the latency classifier (F1 = 0.47) and the request type classifier (F1 = 0.75) confirms that the feature space contains meaningful structure for categorizing \textit{what} a request is, but not for predicting \textit{how long} it will take. This distinction is important for scheduling: request types can inform priority assignment, but they cannot predict execution time under GPU batching.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{algorithm_comparison_bar.png}}
\caption{Average turnaround time for top 8 out of 16 simulated algorithms. FIFO (green) achieves the lowest turnaround whereas all other reordering strategies perform equal or worse.}
\label{fig:simulation}
\end{figure}

\textbf{Key finding:} FIFO moreover matches or outperforms all 16 algorithms on average turnaround time. This result is consistent across all batch windows of 1s, 3s, and 5s. A Welch's $t$-test between FIFO and SJF per request turnaround times yields $p = 0.06$, and bootstrap confidence intervals (for 1,000 resamples) for the mean turnaround difference overlap zero. Neither test reaches significance at $\alpha = 0.05$ thereby confirming that reordering provides no reliable benefit.

\subsection{Per-Class Tradeoff: The Hidden Dimension}

While FIFO wins on an average, the per class analysis reveals a extremely significant tradeoff that the overall metric obscures:
...
\begin{tabular}{lrrrl}
\toprule
\textbf{Algorithm} & \textbf{Fast} & \textbf{Medium} & \textbf{Large} & \textbf{Tradeoff} \\
\midrule
...
SJF reduces fast-task wait by 68\% (5,400 $\rightarrow$ 1,711ms) at the cost of nearly doubling large-task wait (6,223 $\rightarrow$ 12,041ms). This tradeoff is meaningful: in systems where a QA chatbot requires sub 3 second responses while a background code generator can only tolerate 15 seconds, Thus , we conclude that Priority scheduling is strictly superior to FIFO.

\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{perclass_comparison.png}}
...
Selection requires a single configuration change (\texttt{scheduler\_type} in \texttt{config.yaml}) where no code modification is needed.
...
Our simulation shows FIFO that wins on average but loses on \textit{fairness}. In systems with heterogeneous latency SLAs e.g., a real time assistant agent, requiring $<$3s response alongside a batch code generation agent tolerating 15s Priority scheduling reduces the assistant's wait time by 73\% at an acceptable cost to the batch agent. The choice between FIFO and Priority is thus an \textit{engineering decision} driven by deployment requirements and not a universal optimum.
...
\begin{itemize}
    ...
    \item \textbf{Similar model sizes:} 7B vs 8B parameters produce only 920ms median difference. Whereas, larger gaps (e.g., 7B vs 70B) would amplify the model name as a feature.
    \item \textbf{Sequential simulation:} Our simulation does not model GPU batching directly. The sequential model provides an upper bound on reordering benefit only if reordering provides no improvement under sequential execution (where theoretically, it should help the most), it will provide even less under batched execution. A batch level simulator modeling GPU kernel execution and token level synchronization is beyond the scope but represents important future work.
    \item \textbf{No preemptive algorithms:} AIOS supports Round Robin with context interruption, but we only evaluate non preemptive strategies  here.
    \item \textbf{Dataset size:} 2,000 systemcalls provide sufficient coverage for our controlled parameter space however they may not be generalized to production workloads with open ended prompt distributions.
\end{itemize}

\subsection{Future Work}
\begin{itemize}
    \item \textbf{Live batch-composition experiments}: Deploy SJF and Priority on the live AIOS kernel to test whether separating fast and slow requests into different batches actually yield improvement beyond what the simulation has predicted.
    \item \textbf{Output length prediction}:However, a lightweight model predicting \textit{actual} generation length (rather than the budget ceiling) could restore SJF's theoretical advantage.
    \item \textbf{Multi GPU scheduling}: With heterogeneous models across GPUs, request aware routing becomes a load balancing problem with an even richer optimization potential.
    \item \textbf{RL-based adaptive scheduling}: A reinforcement learning agent that optimizes dispatch policy from reward signals (for eg: latency, throughput, fairness) could discover non obvious strategies.
\end{itemize}
...
We investigated whether classical OS scheduling assumptions hold for LLM agent systems or not. We found out that they do not. Our three phase study on 2,000 AIOS syscalls across two LLM backends reveals:

\begin{enumerate}
    \item \textbf{Job-size observability fails:} We observed that request features are weak latency predictors (max $\rho = 0.206$) because \texttt{max\_tokens} is a ceiling and the actual output length is unobservable.
    \item \textbf{ML cannot bridge the gap:} The best latency classifier achieves F1 = 0.47, confirming that latency classes are not separable from input features. However, a 4-class request type classifier achieves F1 = 0.75, showing that structural request categories \textit{are} identifiable---the feature space has structure, but it does not predict runtime.
